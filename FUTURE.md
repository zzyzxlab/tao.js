...
- [x] pre-1.0 (spec train follow-ups, PR #66): first-class lifecycle decoration callbacks for the four events ([`TAO-SPEC.md` §4](./TAO-SPEC.md#4-the-dispatch-lifecycle)), inline invoke-all-then-settle (dispatched vs settled as distinct facts; sibling isolation), `freezeDatum` dev decoration ([`TAO-SPEC.md` §2](./TAO-SPEC.md#2-the-datum-contract)) + re-point code JSDoc citations of "ENVELOPE-SPEC.md §9" to [`TAO-SPEC.md` §7](./TAO-SPEC.md#7-the-wire-contract) — on `feat/lifecycle-callbacks`. Remaining: retire Transponder's first-descendant race in favor of declared responses ([`MESH-SPEC.md` §4](./MESH-SPEC.md#4-protocols))

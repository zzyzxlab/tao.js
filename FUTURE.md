...
- [ ] pre-1.0 (spec train follow-ups, PR #66): first-class lifecycle decoration callbacks for the four events (`TAO-SPEC.md` §4); retire Transponder's first-descendant race in favor of declared responses (`MESH-SPEC.md` §4); `freezeDatum` dev decoration (`TAO-SPEC.md` §2)
...
- [ ] implement a Go lib for the network — a mesh node, not a port: build to `TAO-SPEC.md` + `MESH-SPEC.md` (propagation + invocation edges, lifecycle, kits), with a cross-language TCK run as the proof

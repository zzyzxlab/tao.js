...
| **Silent failure of string-keyed dispatch** — a typo'd trigram is a no-op, not an error                  | Converts an agent's most common failure mode (plausible-but-wrong wiring) into an invisible one | **Typed trigram vocabularies** (TS-first): app declares Terms/Actions/Orients (the Space's axes); typos are out-of-Space ([`TAO-SPEC.md` §9](./TAO-SPEC.md#9-the-declared-space-and-protocols)) and become compile errors. Interim: tracer makes no-ops visible (entry recorded, no chain) | ⬜ TS wrapper ([`FUTURE.md`](./FUTURE.md))                                                                  |
| **No training-data priors** — models hallucinate the API                                                 | Every session needs the contract in-context                                                     | **`TAO.md` convention** for apps (the app's TAO — its declared Space and Protocols — as an intentional artifact; [`TAO-SPEC.md` §9](./TAO-SPEC.md#9-the-declared-space-and-protocols)); dense `llms.txt`-style API docs; this repo's [`AGENTS.md`](./AGENTS.md) as the library-side guide  | ◐ [`AGENTS.md`](./AGENTS.md) shipped; `TAO.md` spec/template and docs rewrite on [`FUTURE.md`](./FUTURE.md) |
...
- [ ] **`TAO.md` spec + template + tooling** for consuming apps — the
      per-app TAO artifact (Space + Protocols, [`TAO-SPEC.md` §9](./TAO-SPEC.md#9-the-declared-space-and-protocols)) agents

...
| **A written protocol, not just an implementation** — envelope scopes + trigram + phases as the contract                                      | Cross-language implementations and transports build from the spec, and agents load the contract instead of inferring it | ✅ `TAO-SPEC.md` (the paradigm, standalone) + `MESH-SPEC.md` (the mesh floor); `ENVELOPE-SPEC.md` records the JS engine |

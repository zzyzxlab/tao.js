...
| **Silent failure of string-keyed dispatch** — a typo'd trigram is a no-op, not an error                  | Converts an agent's most common failure mode (plausible-but-wrong wiring) into an invisible one | **Typed trigram vocabularies** (TS-first): app declares Terms/Actions/Orients (the Space's axes); typos are out-of-Space ([`MESH-SPEC.md`](./MESH-SPEC.md) §3) and become compile errors. Interim: tracer makes no-ops visible (entry recorded, no chain) | ⬜ TS wrapper ([`FUTURE.md`](./FUTURE.md))                                                                  |
| **No training-data priors** — models hallucinate the API                                                 | Every session needs the contract in-context                                                     | **`TAO.md` convention** for apps (the app's TAO — its declared Space and Protocols — as an intentional artifact; [`MESH-SPEC.md`](./MESH-SPEC.md) §3); dense `llms.txt`-style API docs; this repo's [`AGENTS.md`](./AGENTS.md) as the library-side guide  | ◐ [`AGENTS.md`](./AGENTS.md) shipped; `TAO.md` spec/template and docs rewrite on [`FUTURE.md`](./FUTURE.md) |
...
| **Dual dispatch surface** — legacy + envelope modes double what an agent must know (and can hallucinate) | Two ways to do things is a prior-free model's trap                                              | §12 legacy retirement: `enter()` + `decorate()` as the entire Network surface                                                                                                                                                                             | ✅ shipped in 0.19 ([`ENVELOPE-SPEC.md`](./ENVELOPE-SPEC.md) §12)                                           |
...
- [ ] **`TAO.md` spec + template + tooling** for consuming apps — the
      per-app TAO artifact (Space + Protocols, [`MESH-SPEC.md`](./MESH-SPEC.md) §3) agents
...
- [x] **Native `envelope.chain` transport** (socket.io/koa): one `traceId`
      across the client→server→client round trip ([`ENVELOPE-SPEC.md`](./ENVELOPE-SPEC.md) §9)
      — shipped in 0.20 with the boundary primitives + TCK
- [x] **0.19 legacy retirement** — one dispatch surface to learn
      ([`ENVELOPE-SPEC.md`](./ENVELOPE-SPEC.md) §12)

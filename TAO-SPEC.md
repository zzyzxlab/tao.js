...
- `MESH-SPEC.md` — the TAO Mesh Profile, layered above this contract
- `ENVELOPE-SPEC.md` — the design record of the JavaScript implementation
...
- **Chains are the paradigm's only ordering mechanism** beyond the phase
  ...
  signals — declarable as Protocols (`MESH-SPEC.md` §4) — never as
...
consequence (`MESH-SPEC.md` §7): a conformant handler cannot distinguish
...
- **Determinism at races**: a concurrent truthy-vs-AppCon race resolves
  ...
  intercepts read live state; `MESH-SPEC.md` §8.) Tooling lints
...
(`ENVELOPE-SPEC.md` §5) — one implementation of this section.
...
- The contract is **structural, not textual**: any codec carrying the
  JSON data model is legal (`MESH-SPEC.md` §7.3); richer datum models are
...
recorded in that engine's design document: `ENVELOPE-SPEC.md` §10 records
